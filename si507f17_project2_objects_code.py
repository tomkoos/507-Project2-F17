...
''' Instructions for each piece to be completed for this project can be found
in the file, below. '''

''' To see whether your problem solutions are passing the tests, you should run
the Python file:
si507f17_project2_objects_tests.py, which should be saved in the same
directory as this file. '''

''' (DO NOT change the name of this file! Make sure to re-save it with the name
si507f17_project2_objects_code.py if you change the name. Otherwise, we will
not be able to grade it!) '''


print("\n*** *** PROJECT 2 *** ***\n")

''' Useful additional references for this part of the homework from outside
class material:
- the iTunes Search API documentation:
- the following chapters from the textbook (also referred to in SI 506):
  https://www.programsinformationpeople.org/runestone/static/publicpy3/
    Classes/ThinkingAboutClasses.html,
  https://www.programsinformationpeople.org/runestone/static/publicpy3/
    Classes/ClassesHoldingData.html,
  https://www.programsinformationpeople.org/runestone/static/publicpy3/
    UsingRESTAPIs/cachingResponses.html
...
definitely need to check out the documentation to understand/recall how the
parameters of this API work:
...
''' Make sure you understand what the function does, how it works, and how you
could invoke it to get data from iTunes Search about e.g. just songs
...
''' You may want to try them out and see what data gets returned, in order to
complete the problems in this project. '''
...
''' For problem 1, you should define a class Media, representing ANY piece of
media you can find on iTunes search. '''
...
# - author
# - itunes_URL
# - itunes_id (e.g. the value of the track ID, whatever the track is in the
#   data... a movie, a song, etc)
...
#   with the iTunes id number for the piece of media (e.g. the track) only in
#   place of "<itunes id>"
# - a special len method, which, for the Media class, returns 0 no matter what.
#   (The length of an audiobook might mean something different from the length
#   of a song, depending on how you want to define them!)
# - a special contains method (for the in operator) which takes one additional
#   input, as all contains methods must, which should always be a string, and
#   checks to see if the string input to this contains method is INSIDE the
#   string representing the title of this piece of media (the title instance
#   variable)
...
class Media:
    def __init__(self, diction):
        ...
        self.time_ms = diction['trackTimeMillis'] if \
            'trackTimeMillis' in diction else 0
...
''' Now, you'll define 2 more different classes, each of which *inherit from*
class Media:
# class Song
# class Movie '''

''' In the class definitions, you can assume a programmer would pass to each
class's constructor only a dictionary that represented the correct media type
(song, movie). '''

''' Below follows a description of how each of these should be different from
the Media parent class. '''
...
the song. (HINT: The data supplies number of milliseconds in the song... How
can you access that data and convert it to seconds?) '''
...
# - genre
# - description (if none, the value of this instance variable should be None)
#   -- NOTE that this might cause some string encoding problems for you to
#   debug!
''' HINT: Check out the Unicode sub-section of the textbook! This is a common
type of Python debugging you'll encounter with real data... but using the right
small amount of code to fix it will solve all your problems. '''

''' Should have the len method overridden to return the number of minutes in
the movie (HINT: The data returns the number of milliseconds in the movie...
how can you convert that to minutes?) '''

''' Should have an additional method called title_words_num that returns an
integer representing the number of words in the movie description. If there is
no movie description, this method should return 0. '''
...
class Movie(Media):
    def __init__(self, diction):
        ...
        self.genre = diction['primaryGenreName']
        self.description = diction['longDescription'].encode('utf-8') \
            if 'longDescription' in diction else None

    def __len__(self):
        return int(self.time_ms / 1000 / 60)

    def title_words_num(self):
        return 0 if self.description is None \
            else len(self.description.split())


# [PROBLEM 3] [150 POINTS]
print("\n***** PROBLEM 3 *****\n")

''' In this problem, you'll write some code to use the definitions you've just
written. '''

''' First, here we have provided some variables which hold data about media
overall, songs, and movies. '''

''' NOTE: (The first time you run this file, data will be cached, so the data
saved in each variable will be the same each time you run the file, as long as
you do not delete your cached data.) '''
...
written, in order to create a list of each media type, including "media"
generally. '''
...
media_list = [Media(diction) for diction in media_samples]
song_list = [Song(diction) for diction in song_samples]
movie_list = [Movie(diction) for diction in movie_samples]
...
# - artist
# - id
# - url (for the itunes url of that thing -- the url to view that track of
#   media on iTunes)
# - length

''' There are no provided tests for this problem -- you should check your CSV
files to see that they fit this description to see if this problem worked
correctly for you. IT IS VERY IMPORTANT THAT YOUR CSV FILES HAVE EXACTLY
THOSE NAMES! '''
...
''' HINT: You may want to think about what code could be generalized here, and
what couldn't, and write a function or two -- that might make your programming
life a little bit easier in the end, even though it will require more thinking
at the beginning! But you do not have to do this. '''

''' HINT #2: *** You MAY add other, non-required, methods to the class
definitions in order to make this easier, if you prefer to! '''

''' It is perfectly fine to write this code in any way, as long as you rely on
instances of the classes you've defined, and the code you write results in 3
correctly formatted CSV files! '''
